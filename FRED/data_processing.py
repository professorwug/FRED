# AUTOGENERATED! DO NOT EDIT! File to edit: 02 Data Utils - Kernels and Diffusion/02d Flow Datasets.ipynb (unless otherwise specified).
...
__all__ = ['affinity_from_flow', 'affinity_matrix_from_pointset_to_pointset', 'affinity_grid_search',
           ...
           'ManifoldWithVectorFieldV2', 'neighborhood_collate_function', 'dataloader_from_ndarray']
...
from sklearn.neighbors import NearestNeighbors
import torch.nn.functional as F
from .data_processing import flashlight_kernel, diffusion_map_from_affinities, flow_neighbors
class ManifoldWithVectorFieldV2(Dataset):
    ...
    def __init__(self, X, velocities, labels, sigma="automatic", flow_strength = 1, prior_embedding = "diffusion map", t_dmap = 1, dmap_coords_to_use = 2, n_neighbors = 5, minibatch_size = 100, nbhd_strategy = "flow neighbors", verbose = False):
        ...
        self.A = flashlight_kernel(self.X, self.velocities, sigma = sigma, flow_strength = flow_strength)
        self.P_graph = F.normalize(self.A, p=1, dim=1)
        # visualize affinity matrix
        plt.imshow(self.A.numpy())
        ...
        # Step 2. Take a diffusion map of the data
        # These will become our 'precomputed distances' which we use to regularize the embedding
        # Could also use PHATE distances, e.g.
        if prior_embedding == "diffusion map":
            self.P_graph_symmetrized = self.P_graph + self.P_graph.T
            diff_map = diffusion_map_from_affinities(
                self.P_graph_symmetrized, t=t_dmap, plot_evals=False
        ...
        # Step 3: This returns a sparse representation of the flow neighborhoods,
        # in the form of two tensors: row, col. The numbers in row specify the index,
        # and the adjacent entries in col are the neighbors of that index.
        if nbhd_strategy == "flow neighbors":
            self.neighborhoods = flow_neighbors(self.n_nodes, self.P_graph, self.n_neighbors)
        if nbhd_strategy == "euclidean neighbors":
            Neighbors = NearestNeighbors(n_neighbors=self.n_neighbors).fit(self.X)
    ...
    def __len__(self):
        return len(self.X)
    def __getitem__(self, idx):
        # Get the neighborhood around the central point, and randomly choose one neighbor.
        nbhd_idxs = self.neighbors_from_point(idx)
        random_neighbor_idx = np.random.choice(nbhd_idxs)
        # Embed these into a dictionary for easy cross reference
        return_dict = {
            "X":self.X[[idx,random_neighbor_idx]],
            "precomputed distances": self.precomputed_distances,
            "labels":self.labels[[idx, random_neighbor_idx]],
            "indices":torch.tensor([idx,random_neighbor_idx]),
        }
        return return_dict
...
def neighborhood_collate_function(batch):
    # Takes a batch of dictionaries with parameters X, precomputed_distances, and labels, where
    # each point is a point-neighbor pair, and shuffles the points to all lie within the first m/2
    # and neighbors all within the last m/2 indices
    # returns batched dictionary, but with the additional argument "neighbors", a list where neighbor[i] is the index of the
    # neighbor of point i in the batch (always greater than m/2)
    new_batch = {}
    collated_dict = default_collate(batch)
    all_points = collated_dict['X'][:,0]
    all_neighbors = collated_dict['X'][:,1]
    random_neighbor_arrangement = torch.randperm(len(batch))
    permuted_neighbors = all_neighbors[random_neighbor_arrangement]
    neighbor_to_point_idxs = torch.argsort(random_neighbor_arrangement)
    new_batch['X'] = torch.concat([all_points,permuted_neighbors],dim=0)
    new_batch['neighbor'] = torch.concat([random_neighbor_arrangement+len(batch),neighbor_to_point_idxs],dim=0)
    all_labels = collated_dict['labels'][:,0]
    all_neighbor_labels = collated_dict['labels'][:,1]
    permuted_labels = all_neighbor_labels[random_neighbor_arrangement]
    new_batch['labels'] = torch.concat([all_labels,permuted_labels],dim=0)
    # Give the indices of each point in the batch
    all_indices = collated_dict['indices'][:,0]
    all_neighbor_indices = collated_dict['indices'][:,1]
    permuted_indices = all_neighbor_indices[random_neighbor_arrangement]
    new_batch['indices'] = torch.concat([all_indices, permuted_indices],dim=0)
    new_batch['precomputed distances'] = collated_dict['precomputed distances'][0][new_batch['indices']][:,new_batch['indices']]
    return new_batch
...
def dataloader_from_ndarray(X, flow, labels):
    ...
    dataloader = DataLoader(ds, batch_size=32, shuffle=True, collate_fn=neighborhood_collate_function)
    return dataloader